        ...
        foreach (XmlElement node in root.ChildNodes)
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (null != strValue) // 빈 문자열이 있을 수 깄기 때문에 null 확인만 한다.
                {
                    ...
                    if (System.Enum.TryParse<FIRETYPE>(strValue, out FIRETYPE WT)) value = WT;
                    else if (System.Enum.TryParse<WEAPONTYPE>(strValue, out WEAPONTYPE FT)) value = FT;
                    else if (System.Enum.TryParse<ITEMTYPE>(strValue, out ITEMTYPE IT)) value = IT;
                    else if (System.Enum.TryParse<CONSUMPTIONTYPE>(strValue, out CONSUMPTIONTYPE CT)) value = CT;
                    else if (System.Enum.TryParse<SKILLTYPE>(strValue, out SKILLTYPE ST)) value = ST;
